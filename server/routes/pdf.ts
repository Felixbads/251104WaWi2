/**
 * PDF-Generierungs-Route
 * Diese Route bietet einen eigenständigen Endpunkt für die PDF-Generierung,
 * unabhängig vom bestehenden Code
 */
...
// PDF für eine Bestellung generieren
router.get('/orders/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!orderResult || orderResult.length === 0) {
      console.error(`Bestellung ${orderId} nicht gefunden`);
        ...
        if (item.productId) {
          try {
            ...
            if (productResult && productResult.length > 0) {
              ...
              console.warn(`Kein Produkt mit ID ${item.productId} gefunden für Position ${index+1}`);
            }
          } catch (error) {
    ...
    enrichedItems.forEach((item, index) => {
      console.log(`Position ${index+1}:`, {
        id: item.id,
        produktName: item.productName || (item.productDetails?.name) || "Keine Bezeichnung",
    ...
    // PDF generieren
    const pdfBuffer = await generatePDF(order, enrichedItems, supplier, warehouse, totalAmount);
...
async function generatePDF(order: any, items: any[], supplier: any, warehouse: any, totalAmount: number): Promise<Buffer> {
        ...
        <body>
          ...
          <table>
            ...
            ${items.length > 0 ? items.map((item, index) => {
              // Produktname aus verschiedenen Quellen extrahieren
              ...
              <tr>
                <td colspan="7" style="text-align: center; padding: 20px;">
                  <strong>Keine Produkte gefunden.</strong><br>
                  Es sind keine Bestellpositionen für diese Bestellung vorhanden.
                </td>
              </tr>
            `}
