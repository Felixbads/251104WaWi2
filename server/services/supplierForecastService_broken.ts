...
export async function getAvailableSuppliersForForecast(): Promise<any[]> {
  try {
    ...
    // Extract actual data from query result structure
    const suppliers = suppliersResult[3] || suppliersResult.rows || suppliersResult;
    ...
    for (let i = 0; i < suppliers.length; i++) {
      ...
      const warehouseCountData = warehouseCountResult[3] || warehouseCountResult.rows || warehouseCountResult;
      const warehouseCount = Array.isArray(warehouseCountData) && warehouseCountData[0] ? 
        warehouseCountData[0].warehouse_count || 0 : 0;
    ...
    console.log('Returning enriched suppliers:', enrichedSuppliers);
