...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get(`${API_PREFIX}/transactions/summary`, async (req: Request, res: Response) => {
    try {
      // Get all transactions (with a reasonable limit)
      const allTransactions = await storage.getTransactions(1000);

      // Get all machines to calculate active/total
      ...
      const totalRevenue = allTransactions.reduce((sum, tx) => sum + (tx.price || 0), 0);
      
      // Calculate today's totals
      const todayTransactions = allTransactions.filter(tx => {
        const txDate = new Date(tx.datetime);
        return txDate >= today && txDate <= now;
      });
      const todayTransactionsCount = todayTransactions.length;
      const todayRevenue = todayTransactions.reduce((sum, tx) => sum + (tx.price || 0), 0);
      ...
      const yesterdayTransactionsCount = yesterdayTransactions.length;
      const yesterdayRevenue = yesterdayTransactions.reduce((sum, tx) => sum + (tx.price || 0), 0);
      
      // Calculate weekly totals
      const weeklyTransactions = allTransactions.filter(tx => {
        const txDate = new Date(tx.datetime);
        return txDate >= weekStartDate && txDate <= now;
      });
      const weeklyTransactionsCount = weeklyTransactions.length;
      const weeklyRevenue = weeklyTransactions.reduce((sum, tx) => sum + (tx.price || 0), 0);
      
      // Calculate previous week's totals
      const prevWeekTransactions = allTransactions.filter(tx => {
        const txDate = new Date(tx.datetime);
        return txDate >= prevWeekStartDate && txDate <= prevWeekEndDate;
      });
      const prevWeekTransactionsCount = prevWeekTransactions.length;
      const prevWeekRevenue = prevWeekTransactions.reduce((sum, tx) => sum + (tx.price || 0), 0);
      
      // Calculate trends
      const trendDaily = yesterdayRevenue > 0 
        ? ((todayRevenue - yesterdayRevenue) / yesterdayRevenue * 100) 
        : 0;
      const trendWeekly = prevWeekRevenue > 0 
        ? ((weeklyRevenue - prevWeekRevenue) / prevWeekRevenue * 100) 
        : 0;
      
      // Get active machines
      const machines = await storage.getMachines(500);
      const activeMachines = machines.filter(m => m.status === "active").length;
      const totalMachines = machines.length;
      
      // Get open issues
      const events = await storage.getEvents(100);
      ...
      allTransactions.forEach(tx => {
        if (!tx.productName) return;
        
        const prod = productMap.get(tx.productName) || { count: 0, revenue: 0 };
        prod.count += 1;
        prod.revenue += tx.price || 0;
        productMap.set(tx.productName, prod);
      });
      ...
      // Calculate top machines by revenue
      ...
      allTransactions.forEach(tx => {
        ...
        const machine = machineMap.get(tx.machineId) || { 
          id: parseInt(tx.machineId), 
          name: tx.machineName || "Unknown",
          machineName: tx.machineName || "Unknown",
          revenue: 0, 
          transactions: 0 
        };
        
        machine.transactions += 1;
        machine.revenue += tx.price || 0;
        machineMap.set(tx.machineId, machine);
      });
      ...
      allTransactions.forEach(tx => {
        ...
        const method = paymentMap.get(tx.paymentMethod) || { count: 0, revenue: 0 };
        method.count += 1;
        method.revenue += tx.price || 0;
        paymentMap.set(tx.paymentMethod, method);
      });
      ...
      res.status(500).json({ 
        error: "Failed to generate transactions summary", 
        details: error instanceof Error ? error.message : String(error) 
      });
    }
  });
