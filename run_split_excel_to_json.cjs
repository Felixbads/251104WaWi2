/**
 * Wrapper-Skript für die Excel-zu-JSON-Aufteilung
 * 
 * Dieses Skript steuert die Aufteilung einer großen Excel-Datei in JSON-Chunks
 * und führt automatisch wiederholte Durchläufe durch, bis alle Daten verarbeitet sind.
 */
...
const config = {
  ...
  importScript: './split_excel_to_json.cjs',
  maxRuns: 50,    // Maximale Anzahl der Durchläufe
  waitBetweenRuns: 3000 // Wartezeit zwischen Durchläufen in ms
};

/**
 * Führt einen einzelnen Aufteilungs-Durchlauf aus
 */
function runSplitProcess(runNumber, totalRuns) {
  return new Promise((resolve, reject) => {
    ...
    const process = exec(`node ${config.importScript}`, { maxBuffer: 1024 * 1024 * 10 }, (error, stdout, stderr) => {
      ...
      if (fs.existsSync(config.statusFile)) {
        ...
        console.log('Keine Status-Datei gefunden, erster Durchlauf läuft noch oder Aufteilung ist fehlgeschlagen.');
        resolve({ completed: false, success: true });
      }
    });
  });
...
/**
 * Startet den Aufteilungs-Prozess mit Wiederholungen
 */
async function startSplitProcess(maxRuns) {
  console.log('=== STARTE EXCEL-ZU-JSON AUFTEILUNGSPROZESS ===');
  ...
  if (fs.existsSync(config.statusFile)) {
    try {
      ...
      console.log(`- Chunks erstellt: ${status.currentChunkIndex}`);
      console.log(`- Durchläufe: ${status.currentPass - 1}`);
      console.log(`- Letzte Verarbeitung: ${status.lastProcessed || 'Noch nicht gestartet'}`);
      console.log('');
      
      if (status.completed) {
        console.log('Aufteilung ist bereits vollständig abgeschlossen.');
        return;
      } else if (status.failed) {
        console.log(`Aufteilung ist fehlgeschlagen: ${status.errorMessage}`);
        ...
        if (answer !== 'ja' && answer !== 'j' && answer !== 'yes' && answer !== 'y') {
          console.log('Aufteilungs-Prozess beendet.');
          return;
        }
  ...
  while (runCount < maxRuns) {
    ...
    try {
      const result = await runSplitProcess(runCount, maxRuns);
      
      if (result.completed) {
        if (result.success) {
          console.log(`\nAufteilung erfolgreich nach ${runCount} Durchläufen abgeschlossen!`);
        } else {
          console.log(`\nAufteilung fehlgeschlagen nach ${runCount} Durchläufen: ${result.error}`);
        }
        break;
      } else {
  ...
  if (runCount >= maxRuns) {
    console.log(`\nMaximale Anzahl an Durchläufen (${maxRuns}) erreicht, Aufteilung unvollständig.`);
...
// Starte den Aufteilungs-Prozess
startSplitProcess(config.maxRuns);