...
import { Info, ArrowUpDown, TrendingUp, TrendingDown, Equal, BarChart3, LineChart as LineChartIcon } from 'lucide-react';
...
export default function InventurStatistik() {
  ...
  const { data: countData = [], isLoading: isLoadingCounts } = useQuery<MonthlyStatData[]>({
  ...
  const { data: adjustmentData = [], isLoading: isLoadingAdjustments } = useQuery<AdjustmentData[]>({
    queryKey: ['/api/inventory-counts/statistics/adjustments'],
    staleTime: 30 * 60 * 1000, // 30 Minuten Cache
  });

  // Berechne Gesamtstatistiken
  const totalCounts = countData.reduce((sum, month) => sum + month.count, 0);
  const totalPositions = countData.reduce((sum, month) => sum + month.positions, 0);
  ...
  const totalAdjustments = adjustmentData.reduce((sum, item) => sum + Math.abs(item.adjustment), 0);
  const totalPositiveAdjustments = adjustmentData.reduce((sum, item) => sum + (item.adjustment > 0 ? item.adjustment : 0), 0);
  const totalNegativeAdjustments = adjustmentData.reduce((sum, item) => sum + (item.adjustment < 0 ? Math.abs(item.adjustment) : 0), 0);
  ...
  const monthlyChartData: ChartDataItem[] = countData.map((month) => ({
    name: month.month,
  ...
  const productGroupsData: AdjustmentChartItem[] = adjustmentData.map((item) => ({
  ...
  const simplifiedChartData = monthlyChartData.map(({ name, Inventuren }) => ({
    name: name.substring(0, 3), // Kürzere Namen für mobil
    Inventuren
  }));

  const simplifiedAdjustmentData = productGroupsData
    .sort((a, b) => b.Anpassungen - a.Anpassungen)
    .slice(0, 5); // Nur die Top 5 anzeigen auf Mobilgeräten
  ...
  // Render Adjustment Analysis card 
  const renderAdjustmentCard = () => (
    <Card>
      <CardHeader>
  ...
  const renderMobileView = () => {
    ...
    <div className="block md:hidden">
      ...
      >
        ...
        <TabsContent value="charts" className="pt-4 space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Inventuraktivität</CardTitle>
            </CardHeader>
            <CardContent>
              {isLoadingCounts ? (
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={simplifiedChartData}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="name" />
                    <YAxis />
                    <Tooltip />
                    <Line type="monotone" dataKey="Inventuren" stroke="#8884d8" />
                  </LineChart>
                </ResponsiveContainer>
              )}
            </CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Top 5 Warengruppen</CardTitle>
            </CardHeader>
            <CardContent>
              {isLoadingAdjustments ? (
                <Skeleton className="h-[200px] w-full" />
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={simplifiedAdjustmentData}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="name" />
                    <YAxis />
                    <Tooltip />
