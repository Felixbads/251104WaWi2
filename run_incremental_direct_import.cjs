/**
 * Ausführung des inkrementellen Direktimports aus Excel-Datei
 * 
...
const CONFIG = {
  ...
  delayBetweenRuns: 1000,     // Verzögerung zwischen Durchläufen in ms (1 Sekunde)
...
function runImport() {
  return new Promise((resolve, reject) => {
    ...
    const child = exec(`node ${CONFIG.scriptPath}`, (error, stdout, stderr) => {
      ...
      // console.log(`Standardausgabe: ${stdout}`);
      
      resolve();
    });
    
    // Timeout für die Ausführung (30 Sekunden)
    const timeout = setTimeout(() => {
      console.log('Import-Timeout erreicht, beende Prozess...');
      child.kill();
...
// Hauptfunktion
async function main() {
  console.log(`=== STARTE INKREMENTELLEN DIREKTIMPORT PROZESS ===`);
