...
async function cleanupVendonData() {
  ...
  try {
    ...
    const productsWithoutName: typeof allProducts = [];
    ...
    for (const [normalizedName, group] of duplicateGroups) {
      ...
      try {
        // Wir überspringen die stocks-Aktualisierung, da das Schema offenbar anders ist
    ...
    const validVendonIds = new Set<string>();
    ...
    const stockGroups: Record<string, typeof allMachineStocks> = {};
    ...
    for (const [key, group] of duplicateStockGroups) {
      ...
      console.log(`  - Primärer Eintrag ID ${primaryStock.id} ausgewählt (Menge: ${primaryStock.quantity || 0})`);
