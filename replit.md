...
-   **Machine-Warehouse Assignments Display Fix (Sept 2025)**: Resolved empty data display in MachinesTab component where Vendon ID, Standort (location), and Produkte (products) columns showed "–" instead of actual values. Extended `/api/machine-warehouse-assignments` endpoint to include `vendonId` (from machines.vendon_id), `warehouseLocation` (COALESCE from warehouses.city/address with 'Unbekannt' fallback), `productCount` (placeholder set to 0 pending planogram implementation), and `lastRefillDate` (MAX from refills.datetime). These field additions align with frontend expectations and enable proper data visualization in warehouse management interface.
-   **Expired Products Tracking (Sept 2025)**: Implemented comprehensive expired product tracking and audit logging system to monitor product batches that have exceeded their expiration dates. The system includes: (1) Database schema with `expired_products_log` table storing complete expiry history (product details, batch info, quantities, supplier data, location, reason, estimated value), (2) Secure API endpoint at `/api/warehouse3/warehouses/:warehouseId/expired-products` with full authentication, authorization (requireWarehouseAccess), role-based access control, and audit logging, (3) SQL injection protection via parameter whitelisting for sorting/pagination, (4) Frontend ExpiredProductsList component displaying statistics cards (total entries, quantity expired, unique products, estimated value) and detailed table with product information, batch numbers, quantities, expiry dates, and reasons, (5) German locale formatting for dates and currency (EUR), and (6) Complete data-testid attributes for automated testing. All security vulnerabilities identified during architect review have been resolved, including proper middleware invocation, input sanitization, and NULL-safe SQL aggregations using COALESCE.
