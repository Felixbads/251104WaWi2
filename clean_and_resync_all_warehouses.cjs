 ...
 * Es nutzt das clean_and_resync_warehouse.cjs Skript für die eigentliche Arbeit.
 */
...
function cleanAndResyncWarehouse(warehouseId) {
  ...
  try {
    ...
    const output = execSync(`node clean_and_resync_warehouse.cjs ${warehouseId}`, { encoding: 'utf8' });
    console.log(output);
    return true;
  } catch (error) {
