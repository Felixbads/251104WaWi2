...
export const suppliers = pgTable("suppliers", {
  ...
  return {
    // PERFORMANCE OPTIMIZATION: Status-based filtering for active suppliers
    // Heavily used in supplier-portal and supplier selection queries
...
export const purchaseConditions = pgTable("purchase_conditions", {
  ...
  return {
    // PERFORMANCE OPTIMIZATION: Critical indexes for purchase conditions
    // Product-based queries for supplier product listings
    productIdIdx: index().on(table.productId),
    // Supplier-based queries for supplier portal and product management
    supplierIdIdx: index().on(table.supplierId),
    // Composite index for common product + supplier queries (used together frequently)
    productSupplierIdx: index().on(table.productId, table.supplierId),
  };
});
...
export const orders = pgTable("orders", {
  ...
  return {
    ...
    // Required by Architect for Phase 2.2 - verhindert Race Conditions bei order number generation
    orderNumberIdx: uniqueIndex("orders_order_number_unique_idx").on(table.orderNumber),
    
    // PERFORMANCE OPTIMIZATION: Strategic indexes for frequently queried columns
    ...
    // Date range queries for order reporting
    orderDateIdx: index().on(table.orderDate),
...
export const insertOrderSchema = createInsertSchema(orders).omit({
  id: true,
  orderNumber: true, // Generated by API via DB sequence
  createdAt: true,
  updatedAt: true,
  totalAmount: true // wird aus den Positionen berechnet
});

// CRITICAL FIX: Dedicated Order Creation Schema (not based on insertOrderSchema)
// This schema ONLY includes fields that should be provided by the client
// orderNumber is COMPLETELY EXCLUDED as it's generated by the API
export const orderValidationSchema = z.object({
  ...
  // Optional fields that can be provided by client
  notes: z.string().optional(),
});
...
export const orderItems = pgTable("order_items", {
  ...
  return {
    // PERFORMANCE OPTIMIZATION: Critical index for order items lookups
    // Every order detail query uses this FK - most important index
    orderIdIdx: index().on(table.orderId),
    // Product-based queries for inventory management
    productIdIdx: index().on(table.productId),
